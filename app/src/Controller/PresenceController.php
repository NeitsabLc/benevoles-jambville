...
final class PresenceController extends AbstractController
{
    ...
    public function ajouter(
        ...
        if ('SALARIE_ACCUEIL' === $utilisateur->getRoleMetier()) {
        ...
        if ('compa' === $mode && !$utilisateur->isEquipePilote()) {
        ...
        if ($request->isMethod('POST')) {
            ...
            $accueil = $validationPresence->valider(
                $request->request->getString('date_debut'),
                $request->request->getString('date_fin'),
                $request->request->getString('type_couchage'),
                $request->request->getString('commentaire'),
            );
            $erreurs = [...$erreurs, ...$accueil['erreurs']];
            $dateDebut = $accueil['date_debut'];
            ...
            if ('benevole' === $mode) {
                ...
                if (null === $thematique || !$thematique->isActif()) {
                    $erreurs[] = 'Choisissez une thématique active.';
                } elseif (null !== $dateDebut && null !== $dateFin && !in_array($thematique, $thematiques->findDisponiblesPour($dateDebut, $dateFin), true)) {
                ...
                if (null !== $dateDebut && null !== $dateFin && $benevole instanceof Utilisateur && $inscriptions->chevauchePour($benevole, $dateDebut, $dateFin)) {
                    $erreurs[] = 'Cette personne a déjà une présence sur tout ou partie de cette période.';
                }
                if ([] === $erreurs && null !== $thematique && $benevole instanceof Utilisateur) {
    ...
    public function modifier(
        ...
        if ($request->isMethod('POST')) {
            ...
            if ('benevole' === $mode) {
                ...
                if (null !== $dateDebut && null !== $dateFin && $benevole instanceof Utilisateur && $inscriptions->chevauchePour($benevole, $dateDebut, $dateFin, $inscription)) {
                    $erreurs[] = 'Cette personne a déjà une présence sur tout ou partie de cette période.';
                }
                if ([] === $erreurs && null !== $thematique && $benevole instanceof Utilisateur) {
    ...
    private function verifierDroitGestion(Inscription $inscription, Utilisateur $utilisateur): void
    {
        ...
        $estProprietaire = 'INDIVIDUELLE' === $inscription->getType()
    ...
    private function lireRepasSelectionnes(Request $request): array
    {
        ...
        foreach ($request->request->all('repas') as $date => $types) {
            if (!is_string($date) || !is_array($types) || 1 !== preg_match('/^\d{4}-\d{2}-\d{2}$/', $date)) {
                continue;
            }
