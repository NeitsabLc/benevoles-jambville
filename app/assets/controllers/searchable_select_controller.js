...
export default class extends Controller {
    connect() {
        ...
        this.search.placeholder = this.select.dataset.searchPlaceholder || 'Rechercher…';
        this.search.setAttribute('aria-label', this.select.dataset.searchLabel || 'Rechercher');
        this.empty.textContent = this.select.dataset.emptyLabel || 'Aucun résultat';

        this.isOpen = false;
        this.toggle = () => this.isOpen ? this.close() : this.open();
        this.filter = () => this.render();
        this.sync = () => { this.updateTrigger(); this.render(); };
        this.clickOutside = (event) => { if (!this.wrapper.contains(event.target)) this.close(); };
        this.keydown = (event) => {
            if (event.key === 'Escape') {
                this.close();
                this.trigger.focus();
    ...
    updateTrigger() {
        ...
        this.trigger.textContent = selected?.textContent.trim() || 'Sélectionner un bénévole';
    ...
    render() {
        ...
        const available = [...this.select.options].filter((option) => option.value && !option.hidden && !option.disabled && option.textContent.toLocaleLowerCase('fr').includes(query));
